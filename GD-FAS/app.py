...
class Config:
    def __init__(self):
        ...
        self.backbone = 'clip'
        self.gs = True  # Gradient Scaling (Expected by networks.py)
        self.num_classes = 2  # Live vs Attack

        # 2. Hyperparameters (Must match training logs)
        self.temperature = 0.1
        self.lambda_1 = 0.1
        self.lambda_2 = 0.1
        self.lambda_3 = 0.1

        # 3. Attributes that caused your recent crashes
        # The model architecture needs these to build specific layers
        self.num_domain = 1  # You trained on 1 domain (Custom)
        self.beta = 1.5  # From your training log
        self.params = [1.0, 0.8, 0.1, 1.0]  # [alpha, beta, etc] from training log
    ...
    def load_model(self):
        ...
        if os.path.exists(model_path):
            ...
            try:
                ...
                # --- NEW LOGIC: Handle both Dictionaries and Full Objects ---
                if isinstance(checkpoint, dict):
                    # Case 1: The file is a dictionary (Standard)
                    ...
                    # Case 2: The file is the Model Object itself (Your current error case)
                    print("Detected full model object. Extracting state_dict directly...")
                    model.load_state_dict(checkpoint.state_dict(), strict=False)
                ...
                # Optional: Continue with random weights if loading fails
        else:
